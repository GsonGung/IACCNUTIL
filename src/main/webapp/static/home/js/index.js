(function ($) {
    $.learuntab = {
        ...
        refreshTab: function () {
            ...
            var target = $('.IACCNPRO_iframe[data-id="' + currentId + '"]');
        ...
        activeTab: function () {
            ...
            if (!$(this).hasClass('active')) {
                $('.mainContent .IACCNPRO_iframe').each(function () {
                    if ($(this).data('id') == currentId) {
                        $(this).show().siblings('.IACCNPRO_iframe').hide();
                        return false;
                    }
                });
        ...
        closeOtherTabs: function () {
            $('.page-tabs-content').children("[data-id]").find('.fa-remove').parents('a').not(".active").each(function () {
                $('.IACCNPRO_iframe[data-id="' + $(this).data('id') + '"]').remove();
                $(this).remove();
            });
        ...
        addTab: function () {
            ...
            if (flag) {
                ...
                var str1 = '<iframe class="IACCNPRO_iframe" id="iframe' + dataId + '" name="iframe' + dataId + '"  width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
                $('.mainContent').find('iframe.IACCNPRO_iframe').hide();
        ...
        init: function () {
            ...
            $('.tabCloseAll').on('click', function () {
                ...
                $('.page-tabs-content').children("[data-id]:first").each(function () {
                    $('.IACCNPRO_iframe[data-id="' + $(this).data('id') + '"]').show();
    ...
    $.learunindex = {
        ...
        loadMenu: function () {
        	//menuDataMethod("");
    		$.ajax({
    	        url: "../home/toLoadMenuList.do",
    	        type: 'GET',
    	        cache: false,
    	        async: false,
    	        dataType: "json", //可以是text，如果用text，返回的结果为字符串；如果需要json格式的，可以设置为json
    	        ContentType: "application/json; charset=utf-8",
    	        success: function (data) {
    	        	menuDataMethod(data);
    	        },
    	        error: function (jqXHR, textStatus, errorThrown) {
    	        	parent.alertDialog("温馨提示",'10','50','菜单加载失败,请稍后重试','');
    	        }
    		});
        }
    };
     ...
     * @returns
     */
    function menuDataMethod(menuData){
    	//获取菜单列表
    	var data = menuData.list;
    	var _html = "";
